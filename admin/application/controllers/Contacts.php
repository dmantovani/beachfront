...
class Contacts extends CI_Controller {
	...
	public function index()
	{
		...
		$info =  $this->page_model->get_contacts();
		$data['info']=$info;
	    ...
	    $this->template->write_view('content', 'layout/contacts/list', $data, TRUE); 
	...
	public function save(){
		if (isset($this->session->userdata['logged_in'])) {
				...
				*/
				...
				{
					$data = array(
						'nombre' => $_POST['nombre_'.$lan->{"abr"}.''],
						'descripcion' => $_POST['descripcion_'.$lan->{"abr"}.''],
						'color' => $_POST['color_'.$lan->{"abr"}.''],
                        'background' => $_POST["galeria1_input"],
					...
					if($last_insert_id <= 0) 
					{
						$last_insert_id = $this->page_model->insert_categoria($data);
